  ...
  @autoreleasepool {
    ...
    if (self.binaryLayer || self.delegate) {
      ...
      if (self.delegate) {
        ...
        {
            result = [self.reader decode:[bitmap rotateCounterClockwise] hints:self.hints error:&error];
            if (result) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.delegate captureResult:self result:result];
                });
            ...
            {
                UIImage *pickImage = [UIImage imageWithCGImage:source.image];
                ...
                if (feature.count>0) {
                    //取出探测到的数据
                    CIQRCodeFeature *resultFeature = feature[0];
                    dispatch_async(dispatch_get_main_queue(), ^{
                        ZXResult *result = [[ZXResult alloc]init];
                        [result setValue:resultFeature.messageString forKey:@"text"];
                        [self.delegate captureResult:self result:result];
                    });
                }
//                for (CIQRCodeFeature *result in feature) {
//                    
//                    NSLog(@"二维码:%@",result.messageString);
//                }
            }
        }
